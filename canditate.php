...
">
  ...
  <main class="leaderboard__profiles">
   <?php
        ...
        $query="SELECT * FROM `scheduled` WHERE username = '$username' " ;
        if(isset($_POST['sort']))
        {
            $query="SELECT * FROM `scheduled` WHERE username = '$username' order by  date asc,start_time asc " ;
        }
         if(isset($_POST['searchb']))
        {
            $se=$_POST['search'];
            $query="SELECT * FROM `scheduled` where interviewid in  (select  interviewid from scheduled where username='$username')  and username like '%$se%' ";

        }
         if(isset($_POST['searchd']))
        {
            $da=$_POST['search2'];
            $query="SELECT * FROM `scheduled` WHERE username = '$username' and `date`='$da' " ;
...
margin:auto'>";
   ...
   for($page = 1; $page<= $number_of_page; $page++) {  
    echo '<button class="leaderboard__button" style="width:10px;height:10px"> <a style="color:white" href = "canditate.php?page=' . $page . '">' . $page . ' </a></button>';
