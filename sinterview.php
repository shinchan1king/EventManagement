...
if (isset($_POST['select'])){
	...
	$query = "SELECT start_time,end_time,date FROM `scheduled` WHERE username = '$sel' ";
   ...
   if($boo)
   {
    ...
    if($boo==false && sizeof($_POST['select'])>=2)
        {
            $query="SELECT MAX(interviewid) as interviewid from `scheduled`";
            $result = mysqli_query($con,$query) or die(mysql_error());
            $row=mysqli_fetch_array($result);
            $IID=$row['interviewid']-'0';
            $user=$_SESSION['username'];
            foreach($_POST['select'] as $sel)
            {
                
                $query="INSERT INTO `scheduled` (`username`, `interviewid`, `start_time`, `end_time`, `date`,`createdby`) VALUES ('$sel', $IID+1, '$start','$end','$date','$user')";
                $result   = mysqli_query($con, $query);
            }
            echo " <script>alert('scheduled successfully');
                window.location.replace('sinterview.php');
            </script>";
        }
     else{
           if($time)
           {
            ...
            window.location.replace('sinterview.php');</script>";
           }
		...
		<div class="container-login100" >
			<div class="wrap-login100">
				...
				<form class="login100-form validate-form" action="" method="POST" name="schedule">
                    ...
                    <div class="wrap-input100 validate-input" >
                    <h4 > Start Time</h4>
						<input class="input100" type="time" name="s_time" placeholder="Start Time" required/>
						<span class="focus-input100"></span>
						<span class="symbol-input100">
                    ...
                    <div class="wrap-input100 validate-input" >
                    <h4 > End Time</h4>
						<input class="input100" type="time" name="e_time" placeholder="End Time" required/>
						<span class="focus-input100"></span>
						<span class="symbol-input100">
						</span>
					</div>
